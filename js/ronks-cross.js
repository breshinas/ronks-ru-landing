...
async function searchCrosses() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (
        msg.toLowerCase().includes("артикул") &&
...
function processAnalogData(data, originalPart) {
  const mfMap = new Map(
    (data.manufacturerList?.mf || []).map((m) => [m.i, m.ds]),
  );
  const prodMap = new Map((data.productList?.p || []).map((p) => [p.i, p]));

  const target = originalPart.toLowerCase();
  const analogs = (data.analogList?.a || []).filter(
    (a) => a.ns?.toLowerCase() === target,
  );
...
function renderTableSlice() {
            ...
            .map(
              (i, index) => `
							<tr class="clickable-row" data-href="https://lk.ronks.ru" data-row-index="${index}">
							  ...
							  <td class="copyable-cell" data-copy="${escapeHtml(i.analogPart)}">
								  ${escapeHtml(i.analogPart)}
								  <span class="copy-icon" title="Копировать">
  ...
  // Обработка копирования в ячейке "Аналог: номер"
  document.querySelectorAll(".copyable-cell").forEach((cell) => {
    cell.addEventListener("click", (e) => {
      e.stopPropagation(); // Предотвращаем клик по строке (переход в ЛК)

      const textToCopy = cell.getAttribute("data-copy");
      const tooltip = cell.querySelector(".copy-tooltip");
...
function renderSuppliersTable(page, highlightIndex) {
  ...
  // обработка пагинации
  const pagContainer = suppliersTableContainer.querySelector(
